# ============= CELL 3 — CONFIDENCE TIMELINE (run any time after a race) =============
# Draws the game tape from a saved run: every trace's sliding-window confidence over
...
FILE = runs[-1]                                # newest run — or pick one by hand:
# FILE = "q6_stop_wait_belt.pkl"
...
for t in r["traces"]:
    ...
    if t["status"] == "stopped":               # cut at the line
        ax.plot(xs, ys, color="gray", lw=1.0, alpha=0.7,
                label=label_once(f"cut at the line"))
